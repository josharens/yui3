...
import _node_event_simulate from './node-event-simulate.mjs';
import _plugin from './plugin.mjs';
import _node from './node.mjs';
import _attribute from './attribute.mjs';

export default {
  name: 'node-focusmanager',
  load: _load_module_node_focusmanager,
};
  
function _load_module_node_focusmanager(Y, NAME) {

	Y.use(_attribute);
	Y.use(_node);
	Y.use(_plugin);
	Y.use(_node_event_simulate);
	Y.use(_event_key);
	Y.use(_event_focus);

	/** source: node-focusmanager.js */
